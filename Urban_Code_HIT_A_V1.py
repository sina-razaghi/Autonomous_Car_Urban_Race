...
while(True):  
    ...
    side_pix = function_V3.detect_side(side_mask)
    
    # detecting sign type
    red_sign = function_V3.red_sign_state(red_mask)
    sign = function_V3.detect_sign(frame, hsv_frame)
    if sign == 'left':
        sign_state = 'left'
    elif sign == 'straight':
        sign_state = 'straight'
    ...
    cv2.putText(frame, "horiz_line", (96,195), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 0), 2)
    frame = cv2.rectangle(frame,(96,160),(160,180),(0,255,0),1)
    cv2.putText(frame, "side_line", (0,140), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (0, 255, 255), 2)
    frame = cv2.rectangle(frame,(0,150),(256,190),(0,255,255),1)
    cv2.putText(frame, "horiz_90", (85,120), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 255, 0), 2)
    frame = cv2.rectangle(frame,(96,130),(160,140),(255,255,0),1)

    result = np.concatenate([np.concatenate([frame,cv2.cvtColor(function_V3.region_of_interest(white_mask)*255, cv2.COLOR_GRAY2BGR)], 0),
    ...
    if key == ord('w'):
        frame = car.getImage()
        cv2.imwrite('./urban_frame.png', frame)
