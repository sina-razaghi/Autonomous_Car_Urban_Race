...
while(True):
    frame =cv.imread('./urban_orginal_frame_31.png')
    image = cv.resize(frame, (0, 0), fx = 0.5, fy = 0.5)
    ...
    color_mask =cv.inRange(hsv_img,color_low,color_up) * (1-car_mask)
    ...
    cv.imshow('img_fill',bitwise_frame)
